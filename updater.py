...
REPO = "glebat28-dev/RU-A-program-for-converting-decimals-to-mixed-fractions"
...
def check_for_updates():
    ...
    try:
        ...
        if VERSION not in versions:
            print(f"Вашей версии {Fore.LIGHTWHITE_EX}{VERSION} {Fore.WHITE}нету в релизах репозитория. Последняя версия: {Fore.LIGHTWHITE_EX}{latest_release['tag_name']}")
            return
        elif VERSION == latest_release["tag_name"]:
            print(f"У вас установлена последняя версия: {Fore.LIGHTWHITE_EX}{VERSION}")
            return
        print(f"{Fore.YELLOW}Доступна новая версия: {Fore.LIGHTWHITE_EX}{latest_release['tag_name']}")
        if SKIP_UPDATES:
            print(f"Пропускаю установку обновления. Если вы хотите автоматически скачивать обновления, измените значение "
            ...
            return
        print(f"Скачиваю обновление: {Fore.LIGHTWHITE_EX}{latest_release['html_url']}")
        bytes = download_update(latest_release)
        if bytes:
            if install_update(latest_release, bytes):
                print(f"{Fore.YELLOW}Обновление {Fore.LIGHTWHITE_EX}{latest_release['tag_name']} {Fore.YELLOW}было успешно установлено.")
...
def download_update(release_info: dict) -> bytes:
    ...
    """
    try:
        print(f"Загружаю обновление {release_info['tag_name']}...")
...
def install_update(release_info: dict, content: bytes) -> bool:
    ...
    try:
        print(f"Устанавливаю обновление {release_info['tag_name']}...")
